...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const createHorizontalBarChart = (canvasId, series, mode) => {
        const canvas = document.getElementById(canvasId);
        if (!canvas || !hasSeriesData(series)) return;

        const ctx = canvas.getContext('2d');
        const programLabel = programParticipationMode === 'howLongWithOrg'
            ? 'Response Rate'
            : 'Participation Rate';
        const programTooltipLabel = programParticipationMode === 'howLongWithOrg'
            ? 'Share: '
            : 'Participation: ';
        new Chart(ctx, {
        type: 'bar',
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    cornerRadius: 8,
                    callbacks: {
                        label: (context) => programTooltipLabel + context.parsed.x.toFixed(1) + '%'
                    }
                },
                datalabels: {
                    anchor: 'end',
    ...
    // SLIDE 02 - Program Participation / Tenure Chart(s)
    if (showTenureSplitCharts && Array.isArray(tenureGroups) && tenureGroups.length >= 2) {
        createHorizontalBarChart('tenureChart1', tenureGroups[0], 'howLongWithOrg');
        createHorizontalBarChart('tenureChart2', tenureGroups[1], 'howLongWithOrg');
    } else {
        createHorizontalBarChart('programChart', programParticipationData, programParticipationMode);
    }

    // SLIDE 02 - Age Distribution Chart
    const ageCanvas = document.getElementById('ageChart');
    if (ageCanvas && hasSeriesData(ageData)) {
    ...
    if (genderCanvas && hasSeriesData(genderData)) {
    ...
    const viewpointCanvas = document.getElementById('viewpointChart');
    if (viewpointCanvas && hasSeriesData(viewpointData)) {
        const viewpointCtx = viewpointCanvas.getContext('2d');
